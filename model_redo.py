...
EDv = model_check.compute_estimated_demand(SC, SD)
...
for line in f:
    ...
    per = int(tokens[1])
    step = int(tokens[2])
    nv_observed[seg][step][per] = float(tokens[3])
    uv_observed[seg][step][per] = float(tokens[12])
    vhr[seg][step][per] = float(tokens[13])*L[seg]*Th/float(tokens[3])
f.close()
...
for el_i in xrange(NS): 
    ...
    NV_delta.append([[hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='NV_delta'+str(el_i)+str(el_t)+str(el_p)) for el_p in xrange(P)] for el_t in xrange(S)])
    UV_delta.append([[hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='UV_delta'+str(el_i)+str(el_t)+str(el_p)) for el_p in xrange(P)] for el_t in xrange(S)])
    KBv.append([hcm.addVar(vtype=gbp.GRB.CONTINUOUS, lb=KC, name='KB_'+str(el_i)+'_'+str(el_p)) for el_p in xrange(P)])    
    SFv_avg.append([hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='SFv_avg'+str(el_i)+'_'+str(el_p)) for el_p in xrange(P)])
    NVv_avg.append([hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='NVv_avg'+str(el_i)+'_'+str(el_p)) for el_p in xrange(P)])  
    Kv.append([hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='Kv'+str(el_i)+'_'+str(el_p)) for el_p in xrange(P)])      
    V_delta.append([hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='V_delta'+str(el_i)+'_'+str(el_p)) for el_p in xrange(P)])
    Vhr_delta.append([[hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='Vhr_delta'+str(el_i)+str(el_t)+str(el_p)) for el_p in xrange(P)] for el_t in xrange(S)])
    
################################ Unserved Vehicles Indicator ###################################
I_UVv = [] # Array to hold binary indicator
for el_i in xrange(NS):  # Segment Variable
    I_UVv.append([])
...
def SF(i, t, p):
    if t < 0:
        if p is 0:
            return min(SD[max(i-1,0)][0],SC[max(i-1,0)][0])*(1/Th)
        else:
            return SFv[i][S+t][p-1]
    else:
        return SFv[i][t][p]
...
for el_i in xrange(NS+1):  ## Node variables bookend each segment, thus there are NS+1 nodes
    ...
    DEF_I.append([[hcm.addVar(vtype=gbp.GRB.BINARY, name='DEF_I'+str(el_i)+'_'+str(el_t)+'_'+str(el_p)) for el_p in xrange(P)] for el_t in xrange(S)])    
...
def MF(i, t, p):
    if i < 0:
        ...
        if p is 0:
            return min(SD[max(i-1,0)][0],SC[i][p])*(1/Th)  ## Feasibility issue?
        else:
...
for el_i in xrange(len(Ntilde)): #nee NS
    ...
    ONRO_sMax.append([[hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='ONRO_sMax'+str(el_i)+'_'+str(el_t)+'_'+str(el_p)) for el_p in xrange(P)] for el_t in xrange(S)])
    ONRO_sMin.append([[hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='ONRO_sMin'+str(el_i)+'_'+str(el_t)+'_'+str(el_p)) for el_p in xrange(P)] for el_t in xrange(S)])
...
if capacity_var_case == 1:
    ...
    def func_SC(i,t,p):
        if t < 0:
            ...
            return SC[max(0,min(NS-1, i))][max(p,0)]/Th
########################################################################################################################
################################################## Capacity Case 2 #####################################################
# Assumes underlying capacities are known.  Uses CAF variables.
    ...
    for el_i in xrange(NS): # CAF and Capacity are segment variables
        ...
        #CAFv.append([hcm.addVar(vtype=gbp.GRB.CONTINUOUS, ub = 2.0, lb=0.5, name='CAFv_'+str(el_i)+'_'+str(el_p)) for el_p in xrange(P)])
        # CAF Varies by segment, period, and step (can compute capacity drop)
        #CAFv.append([[hcm.addVar(vtype=gbp.GRB.CONTINUOUS, ub = 2.0, lb=0.5, name='CAFv_'+str(el_i)+'_'+str(el_t)+'_'+str(el_p)) for el_t in xrange(S)] for el_p in xrange(P)])
    hcm.update()
    def func_SC(i,t,p):
        if t < 0:
            return func_SC(i, S+t, p-1)
        else:
            i = max(0,min(NS-1,i))  # To maintain 0<=i<=NS-1 indexing
            p = max(p,0)            
            tempVar = CAFv[i]*SC[i][p]
...
################################################## Capacity Case 3 #####################################################
    ...
    for el_i in xrange(NS):
        SCv.append([hcm.addVar(vtype=gbp.GRB.CONTINUOUS, name='SCv'+str(el_i)+'_'+str(el_p)) for el_p in xrange(P)])
    def func_SC(i, t, p):
        if t < 0:
            return func_SC(i, S+t, p-1)
        if i < 0:
            ...
            return SCv[max(0,min(NS-1, i))][max(p,0)]/Th
    ...
    for el_i in xrange(NS):  # UV and I_UV are segment quantities
        for el_t in xrange(S):
            for (el_p) in xrange(P):
                ########################################### SC*Alpha #############################################################
                if el_i > 0:
...
# Setting objective
if use_full_res:
    hcm.setObjective(gbp.quicksum(gbp.quicksum(gbp.quicksum(Vhr_delta[el_i][el_t][el_p] for el_p in xrange(P)) for el_t in xrange(S)) for el_i in xrange(NS)), gbp.GRB.MINIMIZE)  # +gbp.quicksum(CAFv[el_i] for el_i in xrange(NS))
else:
...
for el_i in xrange(NS):  # NV is a Segment Quantity
...
for el_i in xrange(NS+1):
    if el_i-1 in Ftilde:  # Check if OFR at node. If i is OFR Seg, OFRF is at node i+1 (node at downstream end of segment)
        hcm.addConstr(gbp.quicksum(OFRD[el_i-1][x]/4.0 - gbp.quicksum(OFRF(el_i,tau,x) for tau in xrange(S)) for x in xrange(P)) == 0.0)        
        ofr_i = Ftilde.index(el_i-1) # Convert segment idx to ofr var idx (for DEF_I, OFRF_I)
        for el_p in xrange(P):
            for el_t in xrange(S):                                
                # Upper bound on OFRF based on increment's OFRD and any deficit in OFRF from previous periods/steps
                lhs_sum1 = gbp.quicksum(OFRD[el_i-1][x]/4.0 - gbp.quicksum(OFRF(el_i, tau, x) for tau in xrange(S)) for x in xrange(el_p))  # uses el_p and not el_p-1 since xrange is exclusive
                lhs_sum2 = gbp.quicksum(OFRF(el_i, tau, el_p) - OFRD[el_i-1][el_p]/Th for tau in xrange(el_t)) # uses el_t and not el_t-1 since xrange is exclusive
                hcm.addConstr(OFRF(el_i, el_t, el_p) - OFRD[el_i-1][el_p]/Th - lhs_sum1 + lhs_sum2 <= 0.0)
...
for el_i in xrange(NS+1):
    if el_i in Ntilde:
        onr_i = Ntilde.index(el_i) # Convert segment idx to onr var idx (for ONRO_A, ONRO_I, ONRF_I)
        for el_t in xrange(S):
            for el_p in xrange(P):
                #################################### On-Ramp Input #####################################################
                hcm.addConstr(ONRI[onr_i][el_t][el_p] ==  # ONRI at node i equals
                              ONRD[el_i][el_p]*(1/Th)     # ONR demand at segment i (ONRF is computed for node at upstream end of ONR segment)
                ...
                hcm.addConstr(ONRO[onr_i][el_t][el_p] <= ONRC(el_i, el_t, el_p),
                ...
                hcm.addConstr(ONRO_sMax[onr_i][el_t][el_p] >= ONRO_sMin[onr_i][el_t][el_p] * (1.0/(2 * N[el_i][el_p])))
                ...
                hcm.addConstr(ONRQ(el_i,el_t,el_p) 
                            ==gbp.quicksum(ONRD[el_i][x]/4.0 - gbp.quicksum(ONRF(el_i,tau,x) for tau in xrange(S)) for x in xrange(el_p))
                            + gbp.quicksum(ONRD[el_i][el_p]/Th - ONRF(el_i,tau,el_p) for tau in xrange(el_t)),
...
for el_t in xrange(S):
    for el_p in xrange(P):
        hcm.addConstr(MO1(NS,el_t, el_p) == SC[NS-1][el_p]*(1/Th),
                      name='MO1_'+str(NS)+'_'+str(el_t)+'_'+str(el_p))
print("MO1 done")
########################################################################################################################

################################################ Mainline Output 2 #####################################################
# Step 20: Calculate density of queue on segment
for el_i in xrange(NS):  # KQ is a segment quantity
    for el_t in xrange(S):  # Todo account for "t-1"
        for el_p in xrange(P):
            ########################################### KQ  ###########################################################
            hcm.addConstr(KQ[el_i][el_t][el_p] == KJ*N[el_i][el_p]- (N[el_i][el_p]*(KJ-KC))*SF(el_i,el_t-1,el_p)/(SC[el_i][el_p]/Th),
            ...
            # Using I_UV            
            hcm.addConstr(MO2(el_i, el_t, el_p) - SF(el_i, el_t-1, el_p) + ONRF(el_i, el_t, el_p) - KQ[el_i][el_t][el_p]*L[el_i] + NV(el_i, el_t-1, el_p) <= big_m*(1-I_UV(el_i, el_t-1, el_p)),
                          name='MO2_C1_'+str(el_i)+'_'+str(el_t)+'_'+str(el_p))
            hcm.addConstr(SF(el_i, el_t-1, el_p) - ONRF(el_i, el_t, el_p) + KQ[el_i][el_t][el_p]*L[el_i] - NV(el_i, el_t-1, el_p) - MO2(el_i, el_t, el_p) <= big_m*(1-I_UV(el_i, el_t-1, el_p)),
...
for el_t in xrange(S):
    for el_p in xrange(P):
        hcm.addConstr(MO2(NS,el_t, el_p) == SC[NS-1][el_p]*(1/Th),
                      name='MO2_'+str(NS)+'_'+str(el_t)+'_'+str(el_p))
...
for el_i in xrange(NS):
    ...
    for el_p in xrange(P):
        ...
        if el_p is 0:
            ...
            isFCC = ((SC[el_i][el_p] - ONRD[el_i][el_p]) > (SC[el_i][el_p-1]-ONRD[el_i][el_p-1])) and (SC[el_i][el_p]-ONRD[el_i][el_p] > SD[el_i][el_p])
            front_clearing_queue_present[el_i].append(isFCC)
            if isFCC:
                print(str(el_i)+","+str(el_p))
...
#        # fcc_I0 = 1 -> SC(i,p)-ONRD(i,p)]>[SC(i,p-1)-ONRD(i,p-1) is true
...
# MO3 Constraints
for el_i in xrange(NS):  # Not computed for final node as it is assumed there is not queue there?
    for el_t in xrange(S):
        for el_p in xrange(P):
            ############################################ MO3 #########################################################
            if front_clearing_queue_present[el_i][el_p] is False:
                ...
                hcm.addConstr(MO3(el_i, el_t, el_p) <= MO1(el_i+1, el_t-WTT(el_i, el_p), el_p) - ONRF(el_i, el_t, el_p),
                              name='MO3_m1_'+str(el_i)+'_'+str(el_t)+'_'+str(el_p))
                hcm.addConstr(MO3(el_i, el_t, el_p) <= MO2(el_i+1, el_t-WTT(el_i, el_p), el_p) + OFRF(el_i+1,el_t-WTT(el_i,el_p),el_p) - ONRF(el_i, el_t, el_p),
                              name='MO3_m2_'+str(el_i)+'_'+str(el_t)+'_'+str(el_p))
                hcm.addConstr(MO3(el_i, el_t, el_p) <= MO3(el_i+1, el_t-WTT(el_i, el_p), el_p) + OFRF(el_i+1,el_t-WTT(el_i, el_p),el_p) - ONRF(el_i, el_t, el_p),
                              name='MO3_m3_'+str(el_i)+'_'+str(el_t)+'_'+str(el_p))
                hcm.addConstr(MO3(el_i, el_t, el_p) <= func_SC(el_i, el_t-WTT(el_i, el_p), el_p) - ONRF(el_i, el_t, el_p),
                              name='MO3_m4_'+str(el_i)+'_'+str(el_t)+'_'+str(el_p))
                hcm.addConstr(MO3(el_i, el_t, el_p) <= func_SC(el_i+1, el_t-WTT(el_i, el_p), el_p) + OFRF(el_i+1,el_t-WTT(el_i, el_p),el_p) - ONRF(el_i, el_t, el_p),
                              name='MO3_m5_'+str(el_i)+'_'+str(el_t)+'_'+str(el_p))
for el_t in xrange(S):
    for el_p in xrange(P):
        hcm.addConstr(MO3(NS,el_t, el_p) == SC[NS-1][el_p]*(1/Th),
                      name='MO3_'+str(NS)+'_'+str(el_t)+'_'+str(el_p))
...
M_UV = 10000
uv_zero_tol = 0.01
for el_i in xrange(NS):  # NV and UV are segment quantities
    for el_t in xrange(S):
        for el_p in xrange(P):
            ########################################### NV #############################################################
...
for el_i in xrange(NS):
    for el_p in xrange(P):
        hcm.addConstr(SFv_avg[el_i][el_p] == (Th/S)*gbp.quicksum(SF(el_i,tau,el_p) for tau in xrange(S)),
                      name='SFv_avg_'+str(el_i)+'_'+str(el_p))
        hcm.addConstr(NVv_avg[el_i][el_p] == (1.0/S)*gbp.quicksum(NV(el_i,tau,el_p) for tau in xrange(S)),
                      name='NVv_avg_'+str(el_i)+'_'+str(el_p))
        hcm.addConstr(Kv[el_i][el_p] == NVv_avg[el_i][el_p]/L[el_i],
...
if use_speed_match is True:
    if use_full_res is False:    
        for el_i in xrange(NS):
            for el_p in xrange(P):
                hcm.addConstr(V_delta[el_i][el_p] >= SFv_avg[el_i][el_p] - V[el_i][el_p]*Kv[el_i][el_p],
                            name='Obj_Constr1_'+str(el_i)+'_'+str(el_p))
                hcm.addConstr(V_delta[el_i][el_p] >= V[el_i][el_p]*Kv[el_i][el_p] -SFv_avg[el_i][el_p],
                            name='Obj_Constr2_'+str(el_i)+'_'+str(el_p))
    else:
        for el_i in xrange(NS):
            for el_t in xrange(S):
                for el_p in xrange(P):
                    hcm.addConstr(Vhr_delta[el_i][el_t][el_p] >= SF(el_i,el_t,el_p)*func_L(el_i)*Th - vhr[el_i][el_t][el_p]*NV(el_i,el_t,el_p),
                                  name='Obj_constr1_'+str(el_i)+'_'+str(el_t)+'_'+str(el_p))
                    hcm.addConstr(Vhr_delta[el_i][el_t][el_p] >= vhr[el_i][el_t][el_p]*NV(el_i,el_t,el_p) - SF(el_i,el_t,el_p)*func_L(el_i)*Th,
                                  name='Obj_constr2_'+str(el_i)+'_'+str(el_t)+'_'+str(el_p))
...
for p in xrange(P):
    for i in xrange(NS):
        segFlow[i][p] = (Th/S)*sum([SF(i,tau,p).X for tau in xrange(S)])
        numVeh[i][p] = (1.0/S)*sum([NV(i,tau,p).X for tau in xrange(S)])
        K[i][p] = numVeh[i][p]/L[i]
        U[i][p] = segFlow[i][p]/K[i][p]
        print(str(i)+','+str(p)+','+"{0:.2f}".format(U[i][p])+','+"{0:.2f}".format(V[i][p])+','+"{0:.2f}".format(V_delta[i][p].X)) # +','+"{0:.2f}".format(KBv[i][p].X)+','+"{0:.2f}".format(CAFv[i].X)
