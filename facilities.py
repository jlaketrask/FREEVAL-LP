...
def extract(example_problem):
    ...
    if example_problem is 1:
        ...
        # Simple Test Oversat (queue contained) (no oversat)
        NS = 5  # number of segments
        Stilde = [el for el in xrange(NS)]
        Ftilde = []  # List of OFR segments
        Ntilde = [3]  # List of ONR segments
        ...
        facility_data = read_facility_data_from_file("sto_cq2.csv", NS, P)
        ...
        facility_data = read_facility_data_from_file("sto_cq_nq2.csv", NS, P)
        ...
        facility_data = read_facility_data_from_file("sto_cq3.csv", NS, P)
        ...
        L = [el_l/5280.0 for el_l in L]
        SC = facility_data["SC"]  # Segment capacity of segment i in interval p
        mainline_demand = [2000,6200,2000]
        N = facility_data["NL"]  # Number of open lanes in segment i in interval p
        WS = lambda i, p: SC[i][p]/(N[i][p] * (KJ-KC))  # Wave speed for segment i in interval p
        WTT = lambda i, p: int(round(Th * (L[i]/5280.0/WS(i, p))))  # Wave travel time
        ONRD = facility_data["ONRD"]  # Demand flow rate for ONR at node i in interval p
        ...
        facility_data = read_facility_data_from_file("sto_cq_nq3.csv", NS, P)
        ...
        L = [el_l/5280.0 for el_l in L]
        SC = facility_data["SC"]  # Segment capacity of segment i in interval p
        mainline_demand = [2000,4000,2000]
        N = facility_data["NL"]  # Number of open lanes in segment i in interval p
        WS = lambda i, p: SC[i][p]/(N[i][p] * (KJ-KC))  # Wave speed for segment i in interval p
        WTT = lambda i, p: int(round(Th * (L[i]/5280.0/WS(i, p))))  # Wave travel time
        ONRD = facility_data["ONRD"]  # Demand flow rate for ONR at node i in interval p
        ...
        facility_data = read_facility_data_from_file("simple_test_case_gpex1_small.csv", NS, P)

        # Constants
        KC = 45  # Ideal Density at capacity
        KJ = 190  # Facility-wide jam density
        L = [5280,1500,2280,1500,5280]  # Length of each segment
        L = [el_l/5280.0 for el_l in L]
        SC = facility_data["SC"]  # Segment capacity of segment i in interval p
        ...
        facility_data = read_facility_data_from_file("simple_test_case_gpex1_corr.csv", NS, P)
    ...
    elif example_problem is 18:
        NS = 5  # number of segments
        Stilde = [el for el in xrange(NS)]
        Ftilde = []  # List of OFR segments
        Ntilde = []  # List of ONR segments
        Wtilde = []   # List of Weave segments
        P = 8  # number of time intervals (periods) in the analysis period
        Ptilde = [el for el in xrange(P)]  # List of time intervals
        S = 10  # Number of time steps in a single interval (each step is 15 seconds)
        Ttilde = [el for el in xrange(S)]  # List of time steps
        Th = 4*S  # Number of time steps in 1 hour
        alpha = 0  # %

        facility_data = read_facility_data_from_file("ipdet_3step.csv", NS, P)
        ...
        L = [el_l/5280.0 for el_l in L]
        SC = facility_data["SC"]  # Segment capacity of segment i in interval p
        mainline_demand = [5000, 6200, 7200, 7200, 6700, 6500, 5800, 5000]
        N = facility_data["NL"]  # Number of open lanes in segment i in interval p
        WS = lambda i, p: SC[i][p]/(N[i][p] * (KJ-KC))  # Wave speed for segment i in interval p
        WTT = lambda i, p: int(round(Th * (L[i]/5280.0/WS(i, p))))  # Wave travel time
        ONRD = facility_data["ONRD"]  # Demand flow rate for ONR at node i in interval p
        ...
        facility_data = read_facility_data_from_file("ofrf1_5step.csv", NS, P)
        ...
        L = [el_l/5280.0 for el_l in L]
        SC = facility_data["SC"]  # Segment capacity of segment i in interval p
        mainline_demand = [4000, 6000, 4000, 3500]
        N = facility_data["NL"]  # Number of open lanes in segment i in interval p
        WS = lambda i, p: SC[i][p]/(N[i][p] * (KJ-KC))  # Wave speed for segment i in interval p
        WTT = lambda i, p: int(round(Th * (L[i]/5280.0/WS(i, p))))  # Wave travel time
        ONRD = facility_data["ONRD"]  # Demand flow rate for ONR at node i in interval p
        ...
        return {"NS":NS,"Stilde":Stilde,"Ftilde":Ftilde,"Ntilde":Ntilde,"Wtilde":Wtilde,"P":P,"Ptilde":Ptilde,"S":S,
                "Ttilde":Ttilde,"Th":Th,"alpha":alpha,"KC":KC,"KJ":KJ,"L":L,"SC":SC,"mainline_demand":mainline_demand,
                "N":N,"WS":WS,"WTT":WTT,"ONRD":ONRD,"ONRC":ONRC,"RM":RM,"OFRD":OFRD,"SD":SD,"KB":facility_data["KB"],
                "nvuv_fname":'ofrf1_5step_nvuv.csv'}

    elif example_problem is 20:
        # ofrf2_5step.csv
        NS = 6  # number of segments
        Stilde = [el for el in xrange(NS)]
        Ftilde = [3]  # List of OFR segments
        ...
        facility_data = read_facility_data_from_file("ofrf2_5step.csv", NS, P)
        ...
        L = [el_l/5280.0 for el_l in L]
        SC = facility_data["SC"]  # Segment capacity of segment i in interval p
        mainline_demand = [4000, 5100, 4300, 4000]
        N = facility_data["NL"]  # Number of open lanes in segment i in interval p
        WS = lambda i, p: SC[i][p]/(N[i][p] * (KJ-KC))  # Wave speed for segment i in interval p
        WTT = lambda i, p: int(round(Th * (L[i]/5280.0/WS(i, p))))  # Wave travel time
        ONRD = facility_data["ONRD"]  # Demand flow rate for ONR at node i in interval p
        ...
        return {"NS":NS,"Stilde":Stilde,"Ftilde":Ftilde,"Ntilde":Ntilde,"Wtilde":Wtilde,"P":P,"Ptilde":Ptilde,"S":S,
                "Ttilde":Ttilde,"Th":Th,"alpha":alpha,"KC":KC,"KJ":KJ,"L":L,"SC":SC,"mainline_demand":mainline_demand,
                "N":N,"WS":WS,"WTT":WTT,"ONRD":ONRD,"ONRC":ONRC,"RM":RM,"OFRD":OFRD,"SD":SD,"KB":facility_data["KB"],
                "nvuv_fname":'ofrf2_5step_nvuv.csv', "V": V}
                
    elif example_problem is 21:
        # GP Example 1 (5 Steps per Period)
        NS = 11  # number of segments
        Stilde = [el for el in xrange(NS)]
        Ftilde = [3,5,9]  # List of OFR segments
        Ntilde = [1,5,7]  # List of ONR segments
        ...
        return {"NS":NS,"Stilde":Stilde,"Ftilde":Ftilde,"Ntilde":Ntilde,"Wtilde":Wtilde,"P":P,"Ptilde":Ptilde,"S":S,
                "Ttilde":Ttilde,"Th":Th,"alpha":alpha,"KC":KC,"KJ":KJ,"L":L,"SC":SC,"mainline_demand":mainline_demand,
                "N":N,"WS":WS,"WTT":WTT,"ONRD":ONRD,"ONRC":ONRC,"RM":RM,"OFRD":OFRD,"SD":SD,"KB":facility_data["KB"],
                "nvuv_fname":'gpex1_5step_nvuv.csv'}
