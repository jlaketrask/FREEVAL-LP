...
def MF(i, t, p):
    if i < 0:
        ...
        if p == 0:
            return min(fd.SD[max(i-1,0)][0],fd.SC[i][p])*(1/fd.Th)
        else:
    ...
    for el_t in xrange(fd.S):
        for el_i in xrange(fd.NS+1):
            ...
            if el_i-1 in fd.Ftilde:  # Check if OFR at node. If i is OFR Seg, OFRF is at node i+1 (node at downstream end of segment)
                ...
                #hcm.addConstr(OFRF(el_i, el_t, el_p) <= fd.OFRD[el_i-1][el_p]/fd.Th + ODEF[el_i][el_t][el_p],
            ...
            if el_i in fd.Ntilde:
                ...
                #################################### On-Ramp Input #####################################################
                # ONRI = ONRD + ONRQ
                hcm.addConstr(ONRI[onr_i][el_t][el_p] == fd.ONRD[el_i][el_p]*(1/fd.Th) + ONRQ(el_i, el_t-1, el_p),
