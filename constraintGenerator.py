...
def generate_max_constrs(model, binary_vars, var1, var2, var3, M, M1, M2, label1, label2):
    ...
    model.addConstr(var1 - var3 <= M2*binary_vars[0], name=label1+'_D3'+label2)
    model.addConstr(var3 - var1 <= M2*binary_vars[0], name=label1+'_D4'+label2)
    #model.update()
...
def generate_min_constrs(model, binary_vars, var1, var2, var3, M, M1, M2, label1, label2):
    ...
    model.addConstr(var1 - var3 <= M2*binary_vars[1], name=label1+'_D3'+label2)
    model.addConstr(var3 - var1 <= M2*binary_vars[1], name=label1+'_D4'+label2)
    #model.update()
...
def generate_max_constrs_orig(model, var1, var2, var3, M, M1, M2, label1, label2):
    temp_binary_vars = [model.addVar(vtype=gbp.GRB.BINARY, name=label1+'_I'+label2) for v in xrange(2)]
    model.update()
...
def generate_min_constrs_orig(model, var1, var2, var3, M, M1, M2, label1, label2):
    temp_binary_vars = [model.addVar(vtype=gbp.GRB.BINARY, name=label1+'_I'+label2) for v in xrange(2)]
    model.update()
